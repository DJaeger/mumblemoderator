...
import mumblemoderator.service.audio.AudioOutput;
import mumblemoderator.util.Util;

public class MumbleProtocol {

	MumbleConnection getConnection() {
		return this.conn;
	}
...
//	private final static Logger LOGGER = Logger.getLogger(MumbleProtocol.class.getName());
	...
	public final void setComment(String comment) {
		...
		try {
			...
			commenthash = sha1.digest(comment.getBytes());
		} catch (NoSuchAlgorithmException nsaex) {
			commenthash =new byte[]{ 0x00}; 
			
		}
		
		us.setComment(comment);
		us.setCommentHash(ByteString.copyFrom(commenthash));
		conn.sendTcpMessage(MessageType.UserState, us);
	...
	public final void setMute(boolean value) {
		final UserState.Builder us = UserState.newBuilder();
		us.setSession(currentUser.session);
		us.setActor(currentUser.session);
		us.setSelfMute(value);
		conn.sendTcpMessage(MessageType.UserState, us);

	}

	public final void setDeaf(boolean value) {
		final UserState.Builder us = UserState.newBuilder();
		us.setSession(currentUser.session);
		us.setSelfDeaf(value);
		conn.sendTcpMessage(MessageType.UserState, us);
...
//			LOGGER.info("" + Globals.LOG_TAG + "Received Reject message: " + reject.getReason());
			break;
		case ServerSync:
			...
			if (currentUser != null) {
				Util.getLogger(User.class).log(Level.WARNING, "A second ServerSync received.", currentUser);
			}
			...
			pingThread.start();
//			LOGGER.info("" + Globals.LOG_TAG + ">>> " + t);

			ao = new AudioOutput(new Context(), audioHost);
		...
		case ChannelState:
			...
			if (channel != null) {
				...
				channel.pid = cs.getParent();
				if (cs.getLinksList().size() > 0) {
					channel.links = cs.getLinksList();
				}
				if (cs.getLinksAddList().size() > 0) {
					channel.links.addAll(cs.getLinksAddList());
				}
				if (cs.getLinksRemoveList().size() > 0) {
					channel.links.removeAll(cs.getLinksRemoveList());
				}
				
				
				System.out.println(channel.pid + " : " + channel.name + "(" + channel.id + ")");
				for(int i = 0; i<channel.links.size(); i++) {
					System.out.print(channel.links.get(i) + " : ");
				}
				System.out.println();
...
//			LOGGER.info("" + Globals.LOG_TAG + "MumbleConnection: CryptSetup");
...
//				LOGGER.info("" + Globals.LOG_TAG + "MumbleConnection: Server sending nonce");
				conn.cryptState.setServerNonce(cryptsetup.getServerNonce().toByteArray());
			} else {
				// Server wants our nonce.
//				LOGGER.info("" + Globals.LOG_TAG + "MumbleConnection: Server requesting nonce");
			...
			break;
		default:
//			LOGGER.info("" + Globals.LOG_TAG + "unhandled message type " + t);
		}
	}
	...
	public final void sendUserTextMessage(
			final String message,
			final User user ) {
		final TextMessage.Builder tmb = TextMessage.newBuilder();

		tmb.addSession(user.session);
	...
	public final void sendChannelTextMessage(
			...
			final boolean connectedChannel,
			final boolean lowerChannels ) {
		final TextMessage.Builder tmb = TextMessage.newBuilder();

		tmb.addChannelId(channel.id);
	
		if ((channel.links != null) && (connectedChannel)) {
			Iterator<Integer> it = channel.links.iterator();
			while (it.hasNext()) {
				Integer integer = it.next();
				tmb.addChannelId(integer);
				System.out.println("ChannelId: " + integer);
...
//		}
		...
		if (u == null) {
//			LOGGER.info("" + Globals.LOG_TAG + "User session " + uiSession + " not found!");
		}
...
//				LOGGER.info("" + Globals.LOG_TAG + "Interrupted while waiting for audio thread to end"
//					+ e.getMessage());
//			}
//		}
...
//				LOGGER.info("" + Globals.LOG_TAG + "Interrupted while waiting for ping thread to end" + e.getMessage());
			}
		}
	}
