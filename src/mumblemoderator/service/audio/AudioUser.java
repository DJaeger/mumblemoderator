...
import mumblemoderator.jni.Native;
...
//		celtDecoder = Native.celt_decoder_create(celtMode, 1);
		...
		Util.getLogger(AudioOutput.class).log(Level.INFO, "AudioUser created.");
...
//		Native.celt_decode_float(celtDecoder, data, dataLength, lastFrame);
		copyarray(data, lastFrame);
//		Util.getLogger(AudioUser.class).log(Level.INFO, "Buffered:" + normalBuffer.size());
	...
	private void copyarray(byte[] datain, float[] lastFrameout) {
		if(datain != null) {
			float[] tmp_data = new float[datain.length];
			for (int i = 0; i < tmp_data.length; i++) {
				tmp_data[i] = datain[i]/Byte.MAX_VALUE;

			}
			System.arraycopy(tmp_data,0,lastFrameout,0,datain.length);
