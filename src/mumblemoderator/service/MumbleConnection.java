...
import mumblemoderator.service.MumbleProtocol.MessageType;
...
public class MumbleConnection implements Runnable {
	...
	private final static Logger LOGGER = Logger.getLogger(MumbleConnection.class
			.getName());
	...
	class TcpSocketReader extends MumbleSocketReader {
		...
		public void stop() {
			try {
				...
				LOGGER.info(Globals.LOG_TAG + "Error when closing tcp socket " + e.getMessage());
			}
			super.stop();
		}
	...
	public final void disconnect() {
		synchronized (stateLock) {
			...
			LOGGER.info(Globals.LOG_TAG + "MumbleConnection: disconnect");
			disconnecting = true;
			suppressErrors = true;
			...
			try {
				...
				LOGGER.info(Globals.LOG_TAG + "Error disconnecting TCP socket" + e.getMessage());
			}
			if (udpSocket != null) {
				udpSocket.close();
			}
	...
	public final void run() {
		...
		if (protocol == null) {
			Util.getLogger(MumbleConnection.class).log(Level.SEVERE, "Protocol must not be empty");
			System.exit(2);
		}
		...
		try {
			try {
				LOGGER.info(Globals.LOG_TAG + String.format(
				...
				reportError(String.format(
					"Could not connect to Mumble server (KeyManagementException) \"%s:%s\"",
					host,
					port), e);
			} catch (final NoSuchAlgorithmException e) {
				reportError(String.format(
					"Could not connect to Mumble server (NoSuchAlgorithmException) \"%s:%s\"",
					host,
					port), e);
			} catch (final IOException e) {
				reportError(String.format(
					"Could not connect to Mumble server (IOException) \"%s:%s\"",
					host,
					port), e);
			}
...
//		}
		...
		if (forceUdp || useUdpUntil > System.currentTimeMillis()) {
			if (!usingUdp && !forceUdp) {
				LOGGER.info(Globals.LOG_TAG + "MumbleConnection: UDP enabled");
				usingUdp = true;
			}
			...
			if (usingUdp) {
				LOGGER.info(Globals.LOG_TAG + "MumbleConnection: UDP disabled");
				usingUdp = false;
			}
	...
	private void cleanConnection() {
		...
		if (tcpSocket == null) {
			Util.getLogger(MumbleConnection.class).log(Level.WARNING, "tcpSocket already clean and NULL.");
			return;
		} else {
		...
		if (udpSocket == null) {
			Util.getLogger(MumbleConnection.class).log(Level.WARNING, "udpSocket already clean and NULL.");
			return;
	...
	private void reportError(final String error, final Exception e) {
		if (suppressErrors) {
			LOGGER.info(Globals.LOG_TAG + "Error while disconnecting");
			LOGGER.info(Globals.LOG_TAG + error + e);
			return;
		}
	...
	protected Socket connectTcp() throws NoSuchAlgorithmException,
		...
		LOGGER.info(Globals.LOG_TAG + "TCP/SSL socket opened");
	...
	protected DatagramSocket connectUdp() throws SocketException,
		...
		LOGGER.info(Globals.LOG_TAG + "UDP Socket opened");
